...
char ssid[] = "Gpod";   // your network SSID (name)
...
char server[] = "omni2.herokuapp.com"; // name address for Google (using DNS)
...
bool gps_ready()
{
  ...
  while(!isReady){
    ...
    Serial.print("latfix: ");Serial.println((int)GPS.latitude_fixed);
    show_gps();
    if (GPS.fix){
      show_gps();
      isReady = true;
...
void show_gps(){
    ...
    Serial.print(" quality: "); Serial.println((int)GPS.fixquality);
      ...
      Serial.print(GPS.latitudeDegrees);Serial.print(", ");Serial.println(GPS.longitudeDegrees);
      Serial.print("Speed (knots): "); Serial.println(GPS.speed);Serial.print(GPS.longitudeDegrees);
...
void send_query() {
  ...
  while (true) {
    ...
    // int f_mem = freeMemory();
    int serverResponseCode = 0;
    int maxLinesToSend = 10;
    int linesCounter = 0;
    String postData = "csv=";    

    while (session_file.available()) {

      char chr = (char)session_file.read();
      postData += chr;
      ...
      // send data chunk if lines limit reached
      if(linesCounter >= maxLinesToSend){        
        serverResponseCode = send_request(postData);
        // reset data block and lines count
        postData = "csv=";
        linesCounter = 0;
        // postData += "\r\n";
      }
    ...
    // delete the file if all data was sent successfully
...
int send_request(String dataToSend){
  
  String contentType = "text/csv";
  
  int dataLenght = dataToSend.length();
  ...
  client.sendHeader("Content-Length", dataLenght);
  client.beginBody();
